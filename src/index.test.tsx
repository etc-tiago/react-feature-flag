...
describe("FeatureFlagGate", () => {
	...
	describe("Client-side functionality (without cookieHeader)", () => {
		...
		it("should not re-evaluate when cookieHeader is provided (SSR mode)", () => {
			...
			rerender(
				<FeatureFlagGate
					cookieName="different-flag"
					cookieHeader="test-flag=true"
				>
					<div>Feature enabled</div>
				</FeatureFlagGate>,
			);
	...
	describe("Edge cases and error scenarios", () => {
		...
		it("should handle multiple cookies in header", () => {
			...
			render(
				<FeatureFlagGate
					cookieName="test-flag"
					cookieHeader="test-flag=true; other-flag=false; session=abc123"
				>
					<div>Feature enabled</div>
				</FeatureFlagGate>,
			);
			...
			expect(screen.getByText("Feature enabled")).toBeInTheDocument();
			expect(mockCookieParse).toHaveBeenCalledWith(
				"test-flag=true; other-flag=false; session=abc123",
			);
		});
		...
		it("should handle special characters in cookie name", () => {
			...
			render(
				<FeatureFlagGate
					cookieName="test-flag-with-dashes"
					cookieHeader="test-flag-with-dashes=true"
				>
					<div>Feature enabled</div>
				</FeatureFlagGate>,
			);
			...
			expect(screen.getByText("Feature enabled")).toBeInTheDocument();
			expect(mockCookieParse).toHaveBeenCalledWith(
				"test-flag-with-dashes=true",
			);
		});

		it("should handle special characters in cookie value", () => {
			mockCookieParse.mockReturnValue({
				"test-flag": "enabled=true&active=yes",
			});

			render(
				<FeatureFlagGate
					cookieName="test-flag"
					cookieHeader="test-flag=enabled%3Dtrue%26active%3Dyes"
				>
					<div>Feature enabled</div>
				</FeatureFlagGate>,
			);
			...
			expect(screen.getByText("Feature enabled")).toBeInTheDocument();
			expect(mockCookieParse).toHaveBeenCalledWith(
				"test-flag=enabled%3Dtrue%26active%3Dyes",
			);
		});
	});
